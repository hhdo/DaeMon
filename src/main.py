...
def train_and_validate(args, model, train_list, valid_list, test_list, num_nodes, num_rels, model_state_file):
    ...
    for epoch in range(args.n_epoch):
        ...
        for future_sample_id in idx:
            ...
            # Note that we are not add reverse edges in 'all_future_triple' as query query_triple
            ...
            for batch in future_loader:
                ...
                pred = parallel_model(history_g_list, batch_future_all)
...
def test(model, test_list, num_rels, num_nodes, mode="train", model_name = None):
    ...
    # test_list type:  [ [ [s,r,o],[s,r,o] ], [ [s,r,o] ],...]
    ...
    for future_sample_id in idx:
        ...
        # future_sample as the future graph index
        future_list = test_list[future_sample_id]
        # get history graph list
        ...
        for batch in future_loader:
            ...
            t_pred = model(history_g_list, t_batch)
            h_pred = model(history_g_list, h_batch)

            pos_h_index, pos_r_index, pos_t_index = batch.t()

            # time_filter Rank
            timef_t_mask, timef_h_mask = utils.strict_negative_mask(time_filter_data, batch[: , [0,2,1]]) # (h,t,r)
...
if __name__ == '__main__':
    ...
    model_name = "bsize:{}-neg:{}-hislen:{}-msg:{}-aggr:{}-path:{}-dim:{}+{}|{}|{}"\
        .format(args.batch_size, args.negative_num, args.history_len, args.message_func, args.aggregate_func, args.mem_pass,
